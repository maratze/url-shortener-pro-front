	...
	<div class="container mx-auto px-6 py-6 max-w-7xl">
				...
				<div class="relative">
					...
					<div v-if="showFilters"
						...
						<div class="p-4 space-y-4">
							...
							<div>
								<label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Sort
									by</label>
		...
		<div v-else-if="filteredLinks.length === 0" class="my-12 flex flex-col items-center justify-center">
			<div class="bg-slate-100 dark:bg-slate-700 rounded-full p-6 mb-4">
				<svg class="h-12 w-12 text-slate-400 dark:text-slate-500" xmlns="http://www.w3.org/2000/svg" fill="none"
					viewBox="0 0 24 24" stroke="currentColor">
					<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
		...
		<!-- Модальные окна -->
		<create-link-modal v-if="showCreateModal" @close="showCreateModal = false" @create="createLink" />

		<edit-link-modal v-if="showEditModal && selectedLink" :link="selectedLink" @close="showEditModal = false"
			@save="updateLink" />

		<delete-link-modal v-if="showDeleteModal && selectedLink" :link="selectedLink" @close="showDeleteModal = false"
			@confirm="deleteLink" />

		<stats-modal v-if="showStatsModal && selectedLink" :link="selectedLink" @close="showStatsModal = false" />
